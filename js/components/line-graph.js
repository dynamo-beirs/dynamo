...
- 
- Tooltip approach:
- Custom HTML tooltips are rendered as a plain <div> overlaid on the graph
- container using absolute positioning. This replaces the previous
- <foreignObject> approach which caused clipping and visibility bugs on
- mobile browsers (especially Safari), where foreignObject overflow and
- GSAP scale transforms interact unpredictably.
  */
...
buildSVG() {
    ...
    // HTML overlay div for custom tooltips — lives outside the SVG to avoid
    // all foreignObject clipping/scaling bugs on mobile browsers.
    this.container.innerHTML = `
        <div class="comparison-graph-container" style="max-width: 800px; margin: 0 auto 3rem; position: relative; aspect-ratio: 800 / 460;">
            <div class="graph-html-tooltip" style="
                position: absolute;
                top: 0; left: 0;
                opacity: 0;
                pointer-events: none;
...
initGSAP() {
    ...
    // Shared HTML tooltip overlay element for this graph instance.
    // Positioned in SVG-coordinate space by converting dragger coordinates
    // to percentage-based CSS left/top on the container.
    const htmlTooltip = this.container.querySelector('.graph-html-tooltip');
    let htmlTooltipVisible = false;

    const showHtmlTooltip = (html, draggerX, draggerY) => {
        if (htmlTooltip.innerHTML !== html) {
            htmlTooltip.innerHTML = html;
        }

        // The SVG viewBox is 800×460. Convert SVG coords to % of container.
        const svgW = 800;
        const svgH = 460;

        const leftPct = (draggerX / svgW) * 100;
        const topPct  = ((draggerY - 140) / svgH) * 100;

        htmlTooltip.style.left            = `${leftPct}%`;
        htmlTooltip.style.top             = `${topPct}%`;
        htmlTooltip.style.transform       = 'translate(-50%, 0) scale(1)';
        htmlTooltip.style.opacity         = '1';
        htmlTooltip.style.pointerEvents   = 'none';
        htmlTooltipVisible = true;
    };

    const hideHtmlTooltip = () => {
        htmlTooltip.style.opacity   = '0';
        htmlTooltip.style.transform = 'translate(-50%, 0) scale(0)';
        htmlTooltipVisible = false;
    };
    ...
    datasets.forEach(key => {
        ...
        function updateGraph() {
            ...
            if (nearest.tooltipHTML) {
                // Hide the SVG default tooltip pill
                els.box.querySelector('.default-tooltip').style.display = 'none';

                // Update the HTML overlay tooltip position on every frame
                if (isPressed || activeDotIndex !== -1) {
                    showHtmlTooltip(nearest.tooltipHTML, dX, dY);
                }

                // Keep the SVG box group moving too (for the connector line)
                boxPos.x = dX - 45;
                boxPos.y = dY - 130;
            } else {
        ...
        const graphPress = () => {
            ...
            // Only animate the SVG box for non-HTML tooltips
            if (!currentNearest || !currentNearest.tooltipHTML) {
                gsap.to(els.box, {
                ...
                // Keep box group invisible — HTML overlay handles display
                gsap.set(els.box, { opacity: 0, scale: 0 });
            }
        };
        ...
        const connectLine = () => {
            if (isPressed || activeDotIndex !== -1) {
                gsap.set(els.connector, {
                    attr: {
                        x1: gsap.getProperty(els.box, 'x') + 45,
